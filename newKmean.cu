...
__device__ inline int find_nearest_point(
            int                 tid,
            int                 numObjects,
            int                 numAttributes,
...
__global__ void findNewClusterIndex(
            int                 numObjects, 
            int                 numAttributes, 
    const   float* __restrict__ attributes, 
            int                 numClusters, 
...
__global__ void updateNewCluster(
            int                 numObjects, 
            int                 numAttributes, 
    const   float* __restrict__ attributes, 
            int                 numClusters, 
...
__global__ void updateCenter(
    ...
    for(int i = blockIdx.x; i < numClusters; i += gridDim.x) {
        ...
        // new update
        if (new_centers_len[i] < threshold) 
            flag[i] = 1;
